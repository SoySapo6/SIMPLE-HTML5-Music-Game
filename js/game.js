...
function loadOsu(file) {
  content = parseOsuString(file);
...
function readOsu() {
  ...
  reader.onload = function (f) {
    ...
    document
      .getElementById("audiodiv")
      .setAttribute(
        "style",
        "text-align: center; position: absolute; margin: auto; top: 0; left: 0; right: 0; bottom: 0; height: 0px;"
      );
    content = this.result;
    loadOsu(content);
  };
...
function calcPOS(tt) {
  ...
  for (let i = p1; i < TPnums; i++) {
    ...
    if (i === TPnums - 1 || content["TimingPoints"][i + 1][0] > time)
      c =
        ((time - content["TimingPoints"][i][0]) * 800) /
        ((scrollDuration * MpB * percent) / baseMpB);
    else
      c =
        ((content["TimingPoints"][i + 1][0] - content["TimingPoints"][i][0]) *
          800) /
        ((scrollDuration * MpB * percent) / baseMpB);
    tpos = tpos + c;
  ...
  for (let i = p1; i < TPnums; i++) {
    ...
    if (i === TPnums - 1 || content["TimingPoints"][i + 1][0] > tt)
      c =
        ((tt - content["TimingPoints"][i][0]) * 800) /
        ((scrollDuration * MpB * percent) / baseMpB);
    else
      c =
        ((content["TimingPoints"][i + 1][0] - content["TimingPoints"][i][0]) *
...
function bt_start_onclick() {
  ...
  document
    ...
    .setAttribute(
      "style",
      "width: " + c.width + "px; margin:0 auto;"
    );
  c.setAttribute(
    "style",
    "border:4px solid #7f7f7f; background: black; " +
      "width: " +
      c.width +
      "px;"
  );
  scrollDuration = Number(
    document.getElementById("scrollDurationInput").value
  );
  offset = Number(document.getElementById("inputOffset").value);
  if (document.getElementById("baseBPM").value !== "")
    baseMpB = 60000 / Number(document.getElementById("baseBPM").value);
  else {
      ...
      if (Number(content["TimingPoints"][i][1]) > 0)
        baseMpB =
          baseMpB < Number(content["TimingPoints"][i][1])
            ? baseMpB
            : Number(content["TimingPoints"][i][1]);
  }
  initKeyListener();
  startTime();
}
...
function draw_Bar_Line() {
  ...
  for (let i = content["TimingPoints"][p][8]; i < BPMnums; i++) {
    ...
    for (
      let tt = BPMs[i][0];
      i === BPMnums - 1 || (i < BPMnums - 1 && tt < BPMs[i + 1][0]);
      tt = tt + measure
    ) {
      a = true;
      if (tt < time) continue;
      let pos = calcPOS(tt);
      if (pos > c.height + noteThick) break;
      ctx.fillStyle = "#FF0000";
      ctx.fillRect(0, c.height - pos + noteThick, c.width, 2);
      a = false;
    }
    if (a) break;
  }
...
function EndingScene() {
  ...
  ctx.fillText(
    "Score : " + String(Score) + "/" + String(EndScore),
    50,
    670 - 50
  );
  ctx.strokeText(
  ...
  ctx.strokeStyle = "#7F7F7F";
  ctx.fillText(
    "Combo : " + String(MaxCombo) + "/" + String(HOnums + LN),
    50,
    735 - 50
  );
  ctx.strokeText(
  ...
  ctx.strokeStyle = "#7F7F7F";
  ctx.fillText(
    "ACC : " + String(Math.round((Score / EndScore) * 10000) / 100) + "%",
    50,
    800 - 50
  );
  ctx.strokeText(
...
function draw_Notes() {
  ...
  for (let Col = 0; Col < 4; Col++) {
    for (let i = LineQueueHead[Col]; i < LineQueueTail[Col]; i++) {
      if (
        Objs[LineQueue[Col][i]]["EndTime"] > 0 &&
        time - Objs[LineQueue[Col][i]]["EndTime"] > 0
      ) {
        if (
          Objs[LineQueue[Col][i]]["Available"] &&
          LineHold[Col] === LineQueue[Col][i]
        ) {
          HitEvent(Col, 0);
          LineHold[Col] = -1;
      ...
      if (Objs[LineQueue[Col][i]]["EndTime"] !== 0) {
        ...
        ctx.fillRect(L, c.height - pos2, width, pos2 - pos + noteThick);
        ctx.strokeRect(L + 2, c.height - pos2 + 2, width - 4, pos2 - pos + noteThick - 4);
      } else {
        if (pos < 0) {
          continue;
        }
        ctx.fillRect(L, c.height - pos, width, noteThick);
      }
    }
  }
  ctx.restore();
...
function draw_Lasers() {
  ...
  for (let Col = 0; Col < 4; Col++) {
    ...
    let Width = 0;
    if (keyLaserTime[Col] === -1) Width = c.width / 4;
    else Width = ((keyLaserTime[Col] - time) * (c.width / 4)) / LaserTime;
    if (Width < 0) {
      keyLaserTime[Col] = 0;
      continue;
    }
    ...
    ctx.globalAlpha = Width / (c.width / 4);
    ctx.fillRect(L + ((c.width / 4) - Width) / 2, c.height - 300, Width, 300);
  }
  ctx.restore();
}
...
function draw_Combo() {
  ...
  ctx.strokeStyle = "#7F7F7F";
  ctx.lineWidth = 4;
  ctx.strokeText(Combo, c.width / 2, c.height / 2 - 100);
  ctx.fillText(Combo, c.width / 2, c.height / 2 - 100);
  ctx.restore();
}
...
function draw_Score() {
  ...
  ctx.strokeStyle = "#7F7F7F";
  ctx.lineWidth = 4;
  ctx.strokeText(Score, c.width - 20, 50);
  ctx.fillText(Score, c.width - 20, 50);
  ctx.restore();
}
...
function draw_ACC() {
  ...
  ctx.strokeText(Accuracy + "%", c.width - 20, 80);
  ctx.fillText(Accuracy + "%", c.width - 20, 80);
  ctx.restore();
}
...
function draw_Judge() {
  ...
  switch (JudgeType) {
    case 0:
      ctx.fillStyle = "#00FFFF";
      ctx.strokeStyle = "#007F7F";
      ctx.fillText("MARAVILLOSO", c.width / 2, c.height / 2);
      ctx.strokeText("MARAVILLOSO", c.width / 2, c.height / 2);
      break;
    case 1:
      ctx.fillStyle = "#FFFF00";
      ctx.strokeStyle = "#7F7F00";
      ctx.fillText("PERFECTO", c.width / 2, c.height / 2);
      ctx.strokeText("PERFECTO", c.width / 2, c.height / 2);
      break;
    case 2:
      ctx.fillStyle = "#00FF00";
      ctx.strokeStyle = "#007F00";
      ctx.fillText("GENIAL", c.width / 2, c.height / 2);
      ctx.strokeText("GENIAL", c.width / 2, c.height / 2);
      break;
    case 3:
      ctx.fillStyle = "#0000FF";
      ctx.strokeStyle = "#00007F";
      ctx.fillText("BIEN", c.width / 2, c.height / 2);
      ctx.strokeText("BIEN", c.width / 2, c.height / 2);
      break;
    case 4:
      ctx.fillStyle = "#FF0000";
      ctx.strokeStyle = "#7F0000";
      ctx.fillText("MAL", c.width / 2, c.height / 2);
      ctx.strokeText("MAL", c.width / 2, c.height / 2);
      break;
    case 5:
      ctx.fillStyle = "#FF0000";
      ctx.strokeStyle = "#7F0000";
      ctx.fillText("FALLO", c.width / 2, c.height / 2);
      ctx.strokeText("FALLO", c.width / 2, c.height / 2);
      break;
    case 6:
      ctx.fillStyle = "#FF0000";
      ctx.strokeStyle = "#7F0000";
...
function draw_FS() {
  ...
  if (FSType) {
    ctx.fillStyle = "#0000FF";
    ctx.strokeStyle = "#00007F";
    ctx.strokeText("RÁPIDO", c.width / 2, c.height / 2 + 30);
    ctx.fillText("RÁPIDO", c.width / 2, c.height / 2 + 30);
  } else {
    ctx.fillStyle = "#FF0000";
    ctx.strokeStyle = "#7F0000";
    ctx.strokeText("LENTO", c.width / 2, c.height / 2 + 30);
    ctx.fillText("LENTO", c.width / 2, c.height / 2 + 30);
  }
